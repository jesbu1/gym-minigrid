            ...
            for seed_dir in os.listdir(os.path.join(location, log_dir, codebook_file)):
                    for inner_file in os.listdir(os.path.join(location, log_dir, codebook_file, seed_dir)):
                        if inner_file.endswith('progress.csv'):
                            ...
                            if codebook_file not in metrics:
                                metrics[codebook_file] = dict(train=[], test=[])
                            if 'train' in log_dir:
                                #metrics[codebook_file]['train'] = dict(rewards=rewards, path_length=path_length)
                                metrics[codebook_file]['train'].append(rewards)
                            else: 
                                metrics[codebook_file]['test'].append(rewards)
                                #metrics[codebook_file]['test'] = dict(rewards=rewards, path_length=path_length)
